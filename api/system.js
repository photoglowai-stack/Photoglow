// --- Endpoint health/diagnostic + config (system, presets, credits)
...
async function handleSystem(req, res, searchParams) {
  res.setHeader("Content-Type", "application/json; charset=utf-8");

  const pathname = parseUrl(req).pathname.replace(/\/$/, "");
  const isPing = pathname.endsWith("/system") && (searchParams.has("ping") || req.method === "HEAD");
...
async function handleCredits(req, res, searchParams) {
  setCORS(req, res, {
  ...
  if (req.method === "GET" && (searchParams.get("health") === "1" || searchParams.get("health") === 1)) {
...
export default async function handler(req, res) {
  ...
  if (pathname.endsWith("/credits")) {
    return handleCredits(req, res, url.searchParams);
  }
  ...
  if (pathname.endsWith("/system") || pathname.endsWith("/ping")) {
    return handleSystem(req, res, url.searchParams);
  }
